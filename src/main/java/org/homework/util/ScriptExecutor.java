...
public class ScriptExecutor {
    ...
    public static void start() {
        ...
        List<String> resourceFiles = sortScript (getResourceFiles ());
    ...
    private static Set<String> getResourceFiles() {
        final Reflections reflections = new Reflections (ScriptExecutor.PACKAGE_NAME, new ResourcesScanner ());
    ...
    private static Integer parseScriptName(String scriptName) {
        try {
            final String nameWithoutPackage = scriptName.substring (PACKAGE_NAME.length () + 1);
